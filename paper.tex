...
\begin{align}\eqn{flow}
  ...
  \text{pkts}.
\end{align}
% The features represented here are packet size distribution, inter-packet interval distribution, \caps{IP} protocol type, source and destination port numbers, and distribution of packet counts.
...
The scatter plots show the first two most significant dimensions of the behavior matrix after reduction via singular value decomposition (\caps{SVD}) and projection onto the unit-sum hyperplane.
...
\caption{Scatter plots of the two most significant dimensions of the feature-frequency representations of traffic samples from the six network traffic traces considered in this paper.} 
\fig{svd}
\end{center}
\vspace{-2em}
\end{figure}
...
\Section{discussion} discusses the meaning and impact of these results and gives context for this research in relation prior work.
